...
function SettingsItem({ label, sub, subRed = false, right, border = true }) {
  return (
    <div className={`flex items-center justify-between py-4 ${border ? 'border-b border-slate-700/50 last:border-0' : ''}`}>
      <div className='flex-1 pr-4'>
        <div className='text-sm font-medium text-slate-200'>{label}</div>
        {sub && (
          <div className={`text-xs mt-0.5 leading-relaxed ${subRed ? 'text-red-400' : 'text-slate-500'}`}>
            {sub}
          </div>
        )}
      </div>
      ...
      <div className='relative bg-slate-800 border border-slate-700 rounded-2xl p-6 w-full max-w-sm shadow-2xl'>
        ...
        <div className='flex items-center gap-3 mb-4'>
          <div className='w-10 h-10 rounded-xl bg-red-500/15 border border-red-500/30 flex items-center justify-center'>
            <LuAlertTriangle size={18} className='text-red-400' />
          </div>
          <div>
            <div className='font-semibold text-slate-100'>Hapus semua data?</div>
            <div className='text-xs text-slate-500 mt-0.5'>Tindakan ini tidak bisa dibatalkan</div>
          </div>
        </div>
        <p className='text-sm text-slate-400 leading-relaxed mb-5'>
          Seluruh riwayat log, analisis AI, dan pengaturan kamu akan dihapus permanen. Kamu tidak bisa memulihkan data ini.
        </p>
        <div className='flex gap-3'>
          <button
            onClick={onClose}
            className='flex-1 py-2.5 rounded-xl border border-slate-600 text-slate-300 text-sm hover:border-slate-500 transition-all'
          >
            Batal
          </button>
          <button
  ...
  return (
    <div className='fixed inset-0 z-50 flex items-center justify-center p-4'>
      ...
      <div className='relative bg-slate-800 border border-slate-700 rounded-2xl p-6 w-full max-w-sm shadow-2xl'>
        ...
        <div className='font-semibold text-slate-100 mb-4'>Ubah Password</div>
        ...
        <div className='flex gap-3'>
          <button onClick={onClose} className='flex-1 py-2.5 rounded-xl border border-slate-600 text-slate-300 text-sm hover:border-slate-500 transition-all'>
            Batal
          </button>
          <button onClick={onClose} className='flex-1 py-2.5 rounded-xl bg-teal-500 hover:bg-teal-400 text-slate-900 text-sm font-semibold transition-all'>
            Simpan
          </button>
        </div>
      </div>
      ...
      <div className='max-w-2xl mx-auto'>
          ...
          <SettingsItem
            ...
            right={
              <select
                ...
                className='bg-slate-700 border border-slate-600 text-slate-200 text-xs rounded-lg px-2 py-1.5 focus:outline-none focus:border-teal-500 transition-all'
              >
        ...
        <SectionCard title='PRIVASI & DATA' icon={LuShield}>
          <SettingsItem
            ...
            sub='Bantu tingkatkan akurasi model AI kami'
            right={<Toggle />}
          />
          <SettingsItem
          ...
          <SettingsItem
            ...
            right={
              <button
                ...
                className='flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg border border-slate-600 text-slate-300 hover:border-slate-500 transition-all'
              >
                Ubah
