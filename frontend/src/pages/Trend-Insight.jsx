...
// 30 hari data untuk tren (stress 1-4)
const trendData = [
...
function stressColor(v) {
  if (v >= 3) return '#f87171'  // red-400
  if (v >= 2) return '#fbbf24'  // amber-400
...
function StatCard({ label, value, sub, color = 'slate', icon: Icon }) {
  const valColor = { teal: 'text-teal-300', amber: 'text-amber-300', red: 'text-red-400', slate: 'text-slate-100' }[color]
  return (
    <div className='bg-slate-800/50 border border-slate-700/40 rounded-2xl p-5'>
      <div className='flex items-start justify-between mb-3'>
        <div className='text-[10px] font-mono text-slate-500 tracking-widest leading-tight'>{label}</div>
        {Icon && <Icon size={14} className='text-slate-600 flex-shrink-0' />}
      </div>
      <div className={`text-3xl font-serif leading-none mb-1.5 ${valColor}`}>{value}</div>
...
/* ─────────────────────────────────────────────
   Tren 20-hari chart (stress + mood)
───────────────────────────────────────────── */
    ...
    <div>
        ...
        {data.map((d, i) => {
          ...
          return (
            <div key={i} className='flex flex-col items-center flex-1 gap-0.5 group relative'>
              {/* Tooltip */}
              <div className='absolute bottom-full mb-1.5 left-1/2 -translate-x-1/2 bg-slate-700 text-slate-200 text-[10px] font-mono px-2 py-1 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-10 pointer-events-none'>
                {d.day}: {val}
                {isExercise && metric === 'stress' ? ' 🏃' : ''}
              </div>
              <div
      ...
      <div className='flex items-center justify-between mt-2'>
        <span className='text-[11px] text-slate-500'>
          Rata-rata: <span className='font-mono text-slate-300'>{avg}</span>
          {metric === 'stress' ? ' / 4' : ' / 10'}
        </span>
        {metric === 'stress' && (
...
/* ─────────────────────────────────────────────
   Per-hari chart (hari dalam seminggu)
───────────────────────────────────────────── */
  ...
  return (
    <div>
      ...
      <div className='flex gap-3 mt-2'>
        {data.map((d, i) => (
          <div key={i} className='flex-1 text-center text-[10px] font-mono text-slate-500'>{d.hari}</div>
        ))}
      </div>
      <div className='flex items-center justify-between mt-3'>
        <div className='text-xs text-slate-400'>
          Paling stress: <span className='text-red-400 font-medium'>Senin</span>
        </div>
        <div className='text-xs text-slate-400'>
          Paling rileks: <span className='text-teal-400 font-medium'>Sabtu</span>
        </div>
      </div>
    </div>
  )
    ...
    <div>
      ...
      <div className='flex gap-3 items-end' style={{ height: 100 }}>
        <div className='flex flex-col justify-between h-full text-right pr-1' style={{ minWidth: 50 }}>
          {[4, 3, 2, 1].map(v => (
            <span key={v} className='text-[9px] font-mono text-slate-600'>Stress {v}</span>
          ))}
        </div>
      ...
      <div className='text-center text-[11px] text-slate-600 mt-2'>Kualitas Tidur (PSQI)</div>
    </div>
  )
}
...
function FaktorRow({ item }) {
  ...
  const barWidth = (absImpact / 1) * 100

  return (
    <div className='flex items-start gap-3 py-3.5 border-b border-slate-700/50 last:border-0'>
      <div className={`w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0 mt-0.5 ${
        isRisiko ? 'bg-red-500/10' : 'bg-teal-500/10'
      }`}>
        <item.icon size={14} className={isRisiko ? 'text-red-400' : 'text-teal-400'} />
      </div>
      <div className='flex-1 min-w-0'>
        <div className='flex items-center justify-between gap-2 mb-1'>
          <span className='text-sm text-slate-200 font-medium'>{item.label}</span>
          <span className={`text-xs font-mono font-semibold flex-shrink-0 ${isRisiko ? 'text-red-400' : 'text-teal-400'}`}>
      ...
      <div className='flex-shrink-0 mt-1'>
        {isRisiko
          ? <LuAlertTriangle size={13} className='text-red-400/60' />
...
function Section({ title, sub, children, light = false }) {
  if (light) {
    return (
      <div className='bg-white/80 backdrop-blur border border-slate-100 shadow-xl shadow-teal-50 rounded-3xl p-6 md:p-7 mb-5'>
        <div className='mb-5'>
  ...
  return (
    <div className='bg-slate-800/40 border border-slate-700/40 rounded-3xl p-5 md:p-6 mb-5'>
      <div className='mb-4'>
...
function Tabs({ tabs, active, onChange }) {
  return (
    <div className='flex gap-1 p-1 bg-slate-800/60 border border-slate-700/40 rounded-2xl mb-6 w-fit'>
      {tabs.map(t => (
        <button
          key={t.id}
    ...
    <MainLayout title='Insights & Tren'>
      <div className='max-w-3xl mx-auto'>
        ...
        >
          ...
          <div className='mt-4 pt-4 border-t border-slate-100 flex items-start gap-2'>
            ...
            <p className='text-xs text-slate-400 leading-relaxed'>
              Korelasi Pearson r = <span className='font-mono text-slate-300'>−0.72</span> (kuat negatif). Artinya tidur buruk sangat konsisten memperburuk stress harianmu.
            </p>
          </div>
        </Section>
        ...
        <div className='grid md:grid-cols-2 gap-5 mb-5'>
          ...
          {/* Faktor pelindung */}
          <div className='bg-teal-500/5 border border-teal-500/15 rounded-3xl p-5'>
            <div className='flex items-center gap-2 mb-4'>
              <div className='w-7 h-7 rounded-xl bg-teal-500/15 flex items-center justify-content-center'>
                <LuShield size={13} className='text-teal-400 mx-auto' />
              </div>
              <div>
                <div className='text-sm font-semibold text-slate-200'>Faktor Pelindung</div>
                <div className='text-[11px] text-slate-500'>Turunkan stress</div>
              </div>
            </div>
          ...
          <div className='bg-red-500/5 border border-red-500/15 rounded-3xl p-5'>
            <div className='flex items-center gap-2 mb-4'>
              <div className='w-7 h-7 rounded-xl bg-red-500/15 flex items-center justify-content-center'>
                <LuAlertTriangle size={13} className='text-red-400 mx-auto' />
              </div>
              <div>
                <div className='text-sm font-semibold text-slate-200'>Faktor Risiko</div>
                <div className='text-[11px] text-slate-500'>Naikkan stress</div>
              </div>
            </div>
        ...
        <div className='bg-white/80 backdrop-blur border border-slate-100 shadow-xl shadow-teal-50 rounded-3xl p-6 md:p-7 mb-5'>
          <div className='flex items-center gap-2.5 mb-5'>
            <div className='w-8 h-8 rounded-xl bg-teal-500/10 border border-teal-500/20 flex items-center justify-center'>
              <LuSparkles size={14} className='text-teal-400' />
            </div>
            <div>
          ...
          <div className='flex flex-col gap-3'>
            {aiInsights.map((ins, i) => (
              <div
                key={i}
                className='bg-slate-800/40 border border-slate-700/30 rounded-2xl px-4 py-3.5 flex items-start gap-3'
              >
          ...
          <div className='mt-4 text-xs text-slate-500 leading-relaxed px-1'>
            * Insight ini bukan diagnosis medis. Dibuat berdasarkan korelasi pola data pribadimu, bukan standar klinis. Jika stress berkepanjangan, pertimbangkan konsultasi profesional.
          </div>
        </div>
